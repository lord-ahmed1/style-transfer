...
class VGG(nn.Module):
    """
    Pre-trained VGG19 model for neural style transfer.
    
    This class loads a pre-trained VGG19 model and modifies it to extract
    ...
    def __init__(self, content_layers, style_layers):
        ...
        for layer in vgg.children():
            if isinstance(layer, nn.Conv2d):
                ...
                # Replace in-place ReLU with non-in-place version to avoid modifying 
                # the original feature maps
                layer = nn.ReLU(inplace=False)
            elif isinstance(layer, nn.MaxPool2d):
                name = f"pool_{conv_count}"
                # Use average pooling instead of max pooling as recommended in the paper
